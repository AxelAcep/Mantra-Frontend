    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <mark
            key={i}
            className="bg-yellow-200 text-slate-900 rounded-px px-0.5"
          >
            {part}
          </mark>
        ) : (
...
export function PenawaranChatPanel({
  ...
  const searchMatches = useMemo(() => {
    ...
    return chats.filter((c) =>
      c.pesan.toLowerCase().includes(searchQuery.toLowerCase().trim()),
    );
  }, [chats, searchQuery]);
  ...
  useEffect(() => {
    if (open && !isLoading && chats.length > 0 && !firstUnreadId) {
      const first = chats.find(
        (c) =>
          c.pegawaiId !== currentPegawaiId &&
          !c.readBy.includes(currentPegawaiId),
      );
  ...
  function handleSend() {
    ...
    if (editingChat) {
      update(
        { chatId: editingChat.id, pesan: pesan.trim() },
        {
          onSuccess: () => {
            handleCancelEdit();
          },
  ...
  return (
    <>
      ...
      {createPortal(
        <div
          className={`fixed top-0 right-0 h-full w-[420px] max-w-full bg-white shadow-2xl z-[110] flex flex-col transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}
        >
          {/* Header */}
              ...
              {searchQuery && (
                <div className="flex items-center gap-1 border-l border-slate-100 ml-1 pl-1 shrink-0">
                  <span className="text-[9px] text-slate-400 px-1">
                    {searchMatches.length > 0 ? currentSearchIndex + 1 : 0}/
              ...
              <div key={date}>
                ...
                <div className="space-y-3">
                  {items.map((chat) => {
                    ...
                    const isNotification = chat.pesan.startsWith(
                      "[SYSTEM_NOTIFICATION]:",
                    );
                    const isEditable =
                      isMe &&
                      new Date().getTime() -
                        ...
                        3600000;
                    const isHighlighted =
                      searchMatches.some((m) => m.id === chat.id) &&
                      currentSearchIndex !== -1 &&
                      searchMatches[currentSearchIndex].id === chat.id;
                      ...
                      <div key={chat.id} className="flex flex-col gap-3">
                        {isFirstUnread && (
                          <div className="flex justify-center items-center gap-3 my-2">
                            ...
                            <span className="bg-red-500 text-white text-[9px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm">
                              Pesan baru
                            </span>
                            <div className="h-px bg-red-100 flex-1" />
                          </div>
                        ...
                        >
                          {/* Avatar */}
                          <div
                            className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 shadow-sm ${avatarColor}`}
                          >
                            {getInitials(chat.pegawai.nama)}
                          </div>

                          <div
                            className={`max-w-[75%] flex flex-col gap-0.5 ${isMe ? "items-end" : "items-start"} min-w-0`}
                          >
                            {!isMe && (
                            ...
                            <div
                              className={`px-3 py-2 rounded-2xl text-[13px] leading-relaxed break-all whitespace-pre-wrap shadow-sm ${
                                isMe
                            ...
                            <div
                              className={`flex items-center gap-2 px-1 ${isMe ? "flex-row-reverse" : ""}`}
                            >
          ...
          <div className="px-4 py-3 border-t border-gray-100 shrink-0 bg-white shadow-[0_-4px_12px_rgba(0,0,0,0.03)] relative z-10">
            {editingChat && (
              <div className="flex items-center justify-between bg-cyan-50 px-3 py-2 rounded-xl border border-cyan-100 mb-3 animate-in fade-in slide-in-from-bottom-2">
                ...
                <button
                  ...
                  className="text-[11px] text-cyan-600 hover:text-cyan-700 font-bold px-2 py-1 rounded-md hover:bg-cyan-100 transition-colors"
                >
                  BATAL
                </button>
              </div>
            ...
            <p className="text-[10px] text-slate-400 mt-2 ml-1 italic font-light">
