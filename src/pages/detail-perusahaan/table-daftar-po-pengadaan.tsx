...
// Menyimpan data statis tetap di dalam berkas seperti permintaan user
const poData = [
  {
    nomor: "PO-2023-001",
...
// Helper functions (tetap dipertahankan agar tidak ada error referensi)
...
export default function DaftarPoPengadaan() {
  // Gunakan array kosong untuk menyembunyikan data secara visual tanpa menghapus poData statis di atas
  const visiblePoData: typeof poData = [];
  ...
  return (
    <Card className="rounded-xl border-slate-200 shadow-sm overflow-hidden py-0! gap-0!">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 border-b border-slate-100 gap-4">
        <h2 className="text-sm font-bold text-slate-800">Daftar PO Pengadaan</h2>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
              ...
              <TableHead className="text-[10px] font-bold text-slate-500 uppercase h-11 text-right w-[5%] pr-4">
                AKSI
              </TableHead>
          ...
          <TableBody>
            {visiblePoData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-slate-400 text-xs">
                  Tidak ada data PO pengadaan.
                </TableCell>
              </TableRow>
            ) : (
              visiblePoData.map((row, index) => {
                ...
                return (
                  ...
                  >
                    <TableCell className={`font-bold text-slate-800 text-xs py-3.5 pl-4 ${shadowClass}`}>
                      {row.nomor}
                    </TableCell>
      ...
      <div className="flex justify-between items-center p-4 border-t border-slate-100 bg-white">
        <p className="text-xs text-slate-500">
          Menampilkan <strong className="text-slate-700">0</strong> dari <strong className="text-slate-700">0</strong> pesanan
        </p>
        ...
        <div className="flex items-center gap-1">
          ...
          <Button variant="ghost" size="sm" className="w-7 h-7 rounded-md bg-slate-100 text-slate-400 p-0 text-xs font-medium" disabled>
            1
          </Button>
          <Button variant="ghost" size="icon" className="w-7 h-7 rounded-md text-slate-400 hover:text-slate-600" disabled>
