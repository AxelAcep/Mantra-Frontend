...
export default function DaftarRiwayat() {
  // Gunakan array kosong untuk menyembunyikan data secara visual tanpa menghapus riwayatData statis di atas
  const visibleRiwayatData: typeof riwayatData = [];
    ...
    <Card className="rounded-xl border-slate-200 shadow-sm overflow-hidden py-0! gap-0!">
      ...
      <CardContent className="p-0">
        <Table>
          ...
          <TableBody>
            {visibleRiwayatData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-8 text-slate-400 text-xs">
                  Tidak ada data riwayat proyek.
                </TableCell>
              </TableRow>
            ) : (
              visibleRiwayatData.map((row, index) => (
                <TableRow
                  key={index}
      ...
      <div className="flex justify-between items-center p-4 border-t border-slate-100 bg-white">
        <p className="text-xs text-slate-500">
          Menampilkan <strong className="text-slate-700">0</strong> dari <strong className="text-slate-700">0</strong> riwayat
        </p>
