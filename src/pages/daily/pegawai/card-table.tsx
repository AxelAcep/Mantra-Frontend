...
import { BadgeCheck, ChevronLeft, ChevronRight, ChevronsUpDown } from "lucide-react"
...
function VerifiedBadge() {
    return (
        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-green-50 border border-green-200 text-green-600 text-[10px] font-semibold uppercase tracking-wide whitespace-nowrap ml-2">
                    ...
                    {sorted.map((item) => {
                        ...
                        return (
                            <TableRow key={item.id}>
                                ...
                                <TableCell className="font-medium text-slate-700 max-w-[200px] truncate" title={item.judul}>{item.judul}</TableCell>
                                <TableCell className="text-sm text-muted-foreground">{KATEGORI_LABEL[item.kategori] ?? item.kategori}</TableCell>
                                <TableCell className="text-sm text-muted-foreground max-w-[150px] truncate" title={item.perusahaan || "-"}>{item.perusahaan || "-"}</TableCell>
                                <TableCell>
